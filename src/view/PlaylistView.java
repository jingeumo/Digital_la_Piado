...
public class PlaylistView {
    ...
    public void playlistPrint(int userNum, ArrayList<MusicDto> musicList) {
        while (true) {
            System.out.println("[플레이리스트]");
            System.out.print(" [1. 많이 들은 음악 순 2. 최근 들은 순 3. 구매한 노래 -1: 돌아가기] : ");
            int playlistchoose = getUserInput();
            if (playlistchoose == 1) {
                displayPopularPlaylist(userNum, musicList);
            } else if (playlistchoose == 2) {
                displayRecentPlaylist(userNum, musicList);
            } else if (playlistchoose == 3) {
                displayPurchasePlaylist(userNum, musicList);
    ...
    void displayPurchasePlaylist(int userNum, ArrayList<MusicDto> musicList) { // 구매한 음악 순
        ArrayList<PlaylistDto> purchasePlaylist = PlaylistController.getInstance().getPurchasePlaylist();
        displayPlaylist(purchasePlaylist, "[ 구매한 음악 ]", userNum, musicList);
    }

    void displayPopularPlaylist(int userNum, ArrayList<MusicDto> musicList) { // 많이 들은 음악
        ArrayList<PlaylistDto> popularPlaylist = PlaylistController.getInstance().getPopularPlaylist();
        displayPlaylist(popularPlaylist, "[ 많이 들은 음악 ]", userNum, musicList);
    }

    void displayRecentPlaylist(int userNum, ArrayList<MusicDto> musicList) { // 최근 들은 음악 순
        ArrayList<PlaylistDto> recentPlaylist = PlaylistController.getInstance().getRecentPlaylist();
        displayPlaylist(recentPlaylist, "[ 최근 들은 음악 ]", userNum, musicList);
    }


    void displayPlaylist(ArrayList<PlaylistDto> playlist, String title, int userNum, ArrayList<MusicDto> musicList) {
